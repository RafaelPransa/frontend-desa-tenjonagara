...
export default function AdminKontak() {
  ...
  const handleDelete = async (id, subjek) => {
    ...
    try {
      ...
      setError(err.message || 'Gagal menghapus pesan kontak.');
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-serif font-bold text-slate-900">Pesan Kontak Masuk</h1>
            <p className="text-xs sm:text-sm text-slate-500">Daftar aspirasi, pertanyaan, dan pesan dari warga desa</p>
          </div>
        </div>
